...
- **The settings page says when the Bluetooth adapter needs setting**: only
  when the host has more than one, such as a USB dongle next to the built-in
  one, and `hciconfig` lists them. Leaving it empty still uses the system
  default.
